...
/**
 * A representation of the server. Its main use it to be able to open a
 * connection to the server sending back a executed method. The server has to
 * choose which type of method to invoke and should support all method in the
 * HTTP specification.
 *
 * @author Anders Nyman
 */
public interface Server {
    ...
    /**
     * Can do any handling needed of a request before the HttpMethod is
     * executed. Example of handling is to wrap the request.
     *
    ...
    /**
     * Can do handling of the response, if needed the server can also return a
     * new HttpServletResponse if a wrapper of the response is needed.
     *
    ...
    /**
     * Returns the URL scheme for the server.
     *
     * @return
     */
    String getScheme();
    ...
    /**
     * Returns the path that we are mapping to. Starting from the servers root
     * the path starts with a / but doesn't end with a /. A mapping to the root
     * results in path being an empty string "".
     *
     * @return The path
     */
    String getPath();
