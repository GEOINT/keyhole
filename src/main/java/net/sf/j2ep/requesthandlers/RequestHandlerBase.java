...
public abstract class RequestHandlerBase implements RequestHandler {
    ...
    /**
     * A set of headers that are not to be set in the request, these headers are
     ...
     */
    @Override
    public abstract HttpMethod process(HttpServletRequest request, String url)
            throws IOException;

    /**
     * Logging element supplied by commons-logging.
     */
    private static final Log log = LogFactory.getLog(RequestHandlerBase.class);
    ...
    protected void setHeaders(HttpMethod method, HttpServletRequest request)
        ...
        while (headers.hasMoreElements()) {
            ...
            boolean isToken = (connectionToken != null
                    && name.equalsIgnoreCase(connectionToken));
     ...
     * @throws HttpException
     */
    private void setProxySpecificHeaders(HttpMethod method,
            HttpServletRequest request) throws HttpException {
        String serverHostName = "jEasyExtensibleProxy";
        try {
            serverHostName = InetAddress.getLocalHost().getHostName();
        } catch (UnknownHostException e) {
            log.error("Couldn't get the hostname needed "
                    + "for headers x-forwarded-server and Via", e);
        }
        ...
        if (originalVia != null) {
            if (originalVia.contains(serverHostName)) {
                log.error("This proxy has already handled the request, "
                        + "will abort.");
                throw new HttpException("Request has "
                        + "a cyclic dependency on this proxy.");
            }
    ...
    /**
     * Adds all the headers in the input to the list of banned headers. The
     * input string should be comma separated e.g. "Server,Connection,Via"
     *
     * This method is normally called by the factory that is using this request
     * handler.
     *
