...
public abstract class RequestHandlerBase implements RequestHandler {
     ...
     * @see
     * net.sf.j2ep.model.RequestHandler#process(javax.servlet.http.HttpServletRequest,
     * java.lang.String)
     */
    @Override
    ...
    private void setProxySpecificHeaders(HttpMethod method, HttpServletRequest request) throws HttpException {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Couldn't get the hostname needed for "
                    + "headers x-forwarded-server and Via", e);
        }
        ...
        if (originalVia != null) {
            if (originalVia.contains(serverHostName)) {
                logger.log(Level.WARNING, "This proxy has already handled the "
                        + "request, will abort.");
                throw new HttpException("Request has a cyclic dependency "
                        + "on this proxy.");
            }
